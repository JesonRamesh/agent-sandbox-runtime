...
- `research-agent.yaml` is the networked agent. It can fetch from an LLM API
...
through files in `/tmp/agent-handoff/`, which keeps the boundary explicit and
preserves per-agent sandbox policy.
