...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function trunc(str: any, n: number): string {
  if (!str) return '';
  str = String(str);
  return str.length > n ? str.slice(0, n) + '\u2026' : str;
}

function formatDelta(seconds: number | null | undefined): string | null {
...
function KflAttemptNode({ data }: { data: any }) {
  return (
    <>
...
function KflVerdictNode({ data }: { data: any }) {
...
function KflOrphanVerdictNode({ data }: { data: any }) {
  const blocked = data.variant === 'blocked';
  return (
    <>
      <Handle type="target" position={Position.Top} />
...
function KflPolicyNode({ data }: { data: any }) {
  return (
...
function KflCircleNode({ data }: { data: any }) {
  return (
    <>
...
function AutoFollow({ nodeCount, nodes, following, containerRef }: { nodeCount: number; nodes: any[]; following: boolean; containerRef: any }) {
...
// ─── Graph builder ────────────────────────────────────────────────────────────
function buildKernelGraph(llmEvents: any[], kernelEvents: any[]) {
  ...
  const sessionMap = new Map<string, { llm: any[]; kernel: any[] }>();
  ...
  sessions.forEach(([sid, { llm: lEvents, kernel: kEvents }]) => {
    ...
    const attempts = kEvents.filter((e: any) => e.type === 'connect_attempt');
    const verdicts = kEvents.filter((e: any) =>
      e.type === 'connect_allowed' || e.type === 'connect_blocked'
    );

    // Per-session bookends from the LLM stream
    const sessionStart = lEvents.find((e: any) => e.type === 'session_start');
    const sessionEnd   = lEvents.find((e: any) => e.type === 'stopped' || e.type === 'crashed');
    ...
    const pairs: { attempt: any; verdict: any }[] = [];
    ...
    const orphanVerdicts = verdicts.filter((v: any) => !usedVerdicts.has(v._id));
    if (attempts.length === 0) orphanVerdicts.push(...verdicts);

    pairs.sort((a, b) => (a.attempt.ts || 0) - (b.attempt.ts || 0));
    orphanVerdicts.sort((a: any, b: any) => (a.ts || 0) - (b.ts || 0));
...
// ─── Summary ribbon ───────────────────────────────────────────────────────────
function computeKernelStats(kernelEvents: any[]) {
...
function KernelSummary({ stats }: { stats: any }) {
...
export default function KernelFlowLayer({ llmEvents, kernelEvents }: FlowLayerProps) {
