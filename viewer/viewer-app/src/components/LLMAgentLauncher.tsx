...
import { runLlmAgent } from '../api/daemonApi';
...
type LauncherState = 'idle' | 'running' | RunResult;

export default function LLMAgentLauncher() {
  const [task, setTask] = useState<string>('Fetch the front page of https://example.com and summarise.');
  ...
  async function handleRun(e: FormEvent<HTMLFormElement>) {
    ...
    setState('running');
    try {
      const r: any = await runLlmAgent(task.trim());
      setState({ ok: true, message: `pid ${r.pid} started — switch to the Workflow tab to watch` });
    } catch (err: any) {
      setState({ ok: false, message: err?.message ?? String(err) });
    }
  ...
  const result: RunResult | null = state !== 'running' && state !== 'idle' ? state : null;
