...
// Inbound LLM event shape left as `any` — fully typed in T12.
type LLMEvent = any;
...
interface Props {
  ...
  injectionTargets?: Set<unknown> | null;
  onDismissAlert?: () => void;
}
...
function trunc(str: string | undefined | null, n: number): string {
  if (!str) return '';
  const s = String(str);
  return s.length > n ? s.slice(0, n) + '…' : s;
}

function getLabel(event: LLMEvent): string | null {
...
function getDetail(event: LLMEvent): string {
  const data = event.data || {};
  switch (event.type) {
    case 'session_start': {
...
export default function LLMPanel({ events, alert, injectionTargets, onDismissAlert }: Props) {
