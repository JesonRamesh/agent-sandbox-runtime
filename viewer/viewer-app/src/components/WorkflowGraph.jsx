...
const KERNEL_X      = 440;
const NODE_GAP      = 140;
const PHASE_GAP     = 180;  // extra gap between logical phases
const START_Y       = 50;
const COL_WIDTH     = 740;
const CARD_WIDTH    = 280;  // default card width
const CARD_WIDTH_LG = 320;  // wider cards for user_input / agent_output
const MATCH_WIN     = 10;   // seconds

// Detect phase boundaries: a "phase change" happens when the event type
...
// ─── Custom node: annotation badge (floating context label) ──────────────────
...
const nodeTypes = { event: EventNode, circle: CircleNode, kernel: KernelNode, annotation: AnnotationNode };

// ─── Auto-follow: pans to keep bottom node visible at fixed zoom ─────────────
...
function AutoFollow({ nodeCount, nodes, following, containerRef }) {
  ...
  useEffect(() => {
    ...
    // Find the bottom-most node by y position
    let bottomNode = nodes[0];
    for (const n of nodes) {
      if ((n.position?.y || 0) > (bottomNode.position?.y || 0)) {
        bottomNode = n;
      }
    }
    ...
    // Center of the bottom node in graph/content space
    ...
    // Map node center to viewport center:
    // screen_x = node_x * zoom + vp.x  =>  vp.x = screen_cx - node_cx * zoom
...
function buildEventData(event, step) {
  ...
  switch (event.type) {
    ...
    case 'tool_call': {
      ...
      // Human-readable detail: show what's being fetched in plain language
      const detail = url
        ? (isFail
            ? `Attempted to reach ${hostname}`
            : `Fetching data from ${hostname}`)
        : (d.tool || 'unknown tool');
    ...
    case 'agent_output': {
      // Show first sentence as a summary
      const text = d.text || '';
...
function convertEventsToGraph(llmEvents, kernelEvents) {
  ...
  allAgents.forEach((agent, agentIdx) => {
    ...
    for (const event of llmList) {
      ...
      // Detect phase change for extra gap
      if (prevId && !isCircle) {
        ...
        if (prevEvent) {
          ...
          if (prevPhase !== curPhase) {
            y += (PHASE_GAP - NODE_GAP); // add the extra spacing
          }
        }
      }
      ...
      if (isCircle) {
        ...
        // Center circle over the card column
        nodes.push({ id, type: 'circle', position: { x: mainX + (CARD_WIDTH - 120) / 2, y }, data: cData });
      } else {
        step += 1;
        const eData = buildEventData(event, step);
        // Wider cards for text-heavy nodes
    ...
    // ── Identify which tool_calls were blocked (for visual contrast) ──────────
    ...
    // ── Mark blocked tool_call nodes with danger className ────────────────────
    for (const node of nodes) {
      if (blockedToolIds.has(node.id)) {
        node.className = 'wf-node--danger';
      }
    }

    // ── Recolor main-chain edges: green after success, red into blocked ───────
    for (const edge of edges) {
      if (!edge.id.startsWith('e-')) continue; // only main-chain edges
      const targetNode = nodes.find((n) => n.id === edge.target);
      if (targetNode && blockedToolIds.has(targetNode.id)) {
        // Edge leading INTO a blocked tool_call → danger styling
        edge.style = { stroke: '#ff4d5e', strokeWidth: 2.5 };
        edge.animated = true;
      } else if (targetNode && targetNode.data?.variant === 'tool-ok') {
        // Edge leading into a successful tool_call → subtle green
    ...
    for (const ke of kernelList) {
      ...
      // Align kernel node vertically with the tool_call card center
      ...
      // Edge from tool_call → kernel node (horizontal connection)
      ...
      // ── Annotation node for blocked connections ─────────────────────────────
      if (variant === 'blocked' && best) {
        const annId = `ann-${ke._id}`;
        // Position above the kernel node, centered horizontally
        const annX  = kernelX + 20;
        const annY  = nodeY - 30;
        nodes.push({
          id: annId, type: 'annotation',
          position: { x: annX, y: annY },
          data: {
            variant: 'danger',
            icon: '⚠',
    ...
    // ── Annotation badges for safe tool_calls (above their kernel node) ───────
    // We need to find the kernel node matched to each safe tool_call
    for (const ke of kernelList) {
      if (ke.type === 'connect_blocked') continue; // handled by danger annotation
      // Find matched tool_call
      let matchedTc = null;
      for (let i = toolCallNodes.length - 1; i >= 0; i--) {
        const tc = toolCallNodes[i];
        const dt = ke.ts - tc.ts;
      ...
      if (matchedTc && !blockedToolIds.has(matchedTc.id)) {
        const annId = `ann-safe-${ke._id}`;
        // Position above the kernel node, same pattern as danger annotation
        const annY = matchedTc.y + 15 - 30; // kernelNodeY - 30
        nodes.push({
          id: annId, type: 'annotation',
          position: { x: kernelX + 20, y: annY },
          data: {
            variant: 'safe',
            icon: '✓',
...
export default function WorkflowGraph({ llmEvents, kernelEvents }) {
  ...
  // When user manually moves the viewport, stop following
  const handleMoveStart = useCallback((event) => {
    // If event exists, it's user-initiated (mouse/touch)
    if (event) {
  ...
  return (
    <div className="wf-wrapper">
      ...
      <ReactFlowProvider>
        ...
        >
          <AutoFollow nodeCount={nodes.length} nodes={nodes} following={following} containerRef={containerRef} />
