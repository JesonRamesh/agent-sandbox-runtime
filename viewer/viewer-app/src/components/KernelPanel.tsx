...
// Inbound event shape left as `any` — fully typed in T12.
type KernelEvent = any;
...
function getLabel(event: KernelEvent): string {
  switch (event.type) {
...
function getDetail(event: KernelEvent): string {
  const d = event.data || {};
  switch (event.type) {
...
export default function KernelPanel({ events }: Props) {
