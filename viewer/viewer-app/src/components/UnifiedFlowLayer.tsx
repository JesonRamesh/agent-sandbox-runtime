...
function preprocessLlm(events: any[]): any[] {
  const filtered = events.filter((e: any) => e.type !== 'stdout');
  const result: any[] = [];
  let i = 0;
...
function buildCircleData(event: any) {
  const d = event.data || {};
  switch (event.type) {
    case 'session_start':
      return {
...
function buildEventData(event: any, step: number) {
  const d = event.data || {};
  switch (event.type) {
    case 'user_input':
      return {
...
// ─── Core conversion ──────────────────────────────────────────────────────────
function convertEventsToGraph(llmEvents: any[], kernelEvents: any[]) {
  const kernel = kernelEvents.filter((e: any) => e.type !== 'connect_attempt');

  const agentLlm    = new Map<string, any[]>();
  const agentKernel = new Map<string, any[]>();
  ...
  allAgents.forEach((agent, agentIdx) => {
    ...
    let step   = 0;
    const toolCallNodes: { id: string; ts: number; y: number }[] = [];
    ...
    // ── Kernel nodes ──────────────────────────────────────────────────────────
    const usedTc = new Map<string, any>();
    for (const ke of kernelList) {
      const kid     = `n-${ke._id}`;
...
function computeSessionStats(llmEvents: any[], kernelEvents: any[]) {
  const filtered = llmEvents.filter((e: any) => e.type !== 'stdout');
...
function SessionSummary({ stats }: { stats: any }) {
...
export default function UnifiedFlowLayer({ llmEvents, kernelEvents }: FlowLayerProps) {
