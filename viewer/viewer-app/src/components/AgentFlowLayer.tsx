...
function preprocessAgentEvents(llmEvents: any[]): any[] {
  // 1. keep only agent-meaningful events
  const filtered = llmEvents.filter((e: any) => e.type !== 'stdout' || isThinkingLine(e));
...
function isThinkingLine(e: any): boolean {
...
function AgentCircleNode({ data }: { data: any }) {
  return (
    <>
...
function AgentTaskNode({ data }: { data: any }) {
  return (
    <>
...
function AgentReasoningNode({ data }: { data: any }) {
  return (
    <>
      <Handle type="target" position={Position.Top} />
...
function AgentToolNode({ data }: { data: any }) {
...
function AgentOutputNode({ data }: { data: any }) {
  return (
    <>
      <Handle type="target" position={Position.Top} />
...
function buildAgentGraph(llmEvents: any[], kernelEvents: any[]) {
  // Group by agent
...
// ─── Summary ribbon ───────────────────────────────────────────────────────────
function computeAgentStats(llmEvents: any[], kernelEvents: any[]) {
  const meaningful = llmEvents.filter((e: any) =>
...
function AgentSummary({ stats }: { stats: any }) {
...
export default function AgentFlowLayer({ llmEvents, kernelEvents }: FlowLayerProps) {
