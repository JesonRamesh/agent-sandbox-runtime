...
async function listEntries(playgroundDir) {
  let names;
  try {
...
async function listAll(playgroundDir) {
...
async function getById(id, playgroundDir) {
...
function validateForCreate(policy) {
...
async function update(id, policy, playgroundDir) {
  if (!Number.isInteger(id) || id < 1) {
    throw new PolicyStoreError('invalid', 'id must be a positive integer');
  }
  if (typeof policy.name !== 'string' || policy.name.trim() === '') {
...
async function remove(id, playgroundDir) {
  if (!Number.isInteger(id) || id < 1) {
    throw new PolicyStoreError('invalid', 'id must be a positive integer');
  }
  const entries = await listEntries(playgroundDir);
...
async function nextId(playgroundDir) {
  const entries = await listEntries(playgroundDir);
...
async function scenarioStemForId(id, playgroundDir) {
  const entries = await listEntries(playgroundDir);
  const match = entries.find((e) => e.id === id);
  if (!match) throw new PolicyStoreError('not_found', `policy id=${id} not found`);
