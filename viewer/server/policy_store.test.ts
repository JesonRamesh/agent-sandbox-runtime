...
test('listAll returns sorted policies and skips non-policy files', async () => {
  ...
  assert.deepEqual(out.map((p: any) => p.id), [1, 2, 3]);
...
test('getById throws not_found for unknown id', async () => {
  ...
  await assert.rejects(
    () => getById(99, dir),
    (err: any) => err instanceof PolicyStoreError && err.code === 'not_found',
  );
});
...
test('create rejects duplicate ids', async () => {
  ...
  await assert.rejects(
    ...
    (err: any) => err instanceof PolicyStoreError && err.code === 'conflict',
  );
});
...
test('create rejects invalid names', async () => {
  ...
  await assert.rejects(
    ...
    (err: any) => err instanceof PolicyStoreError && err.code === 'invalid',
  );
});
