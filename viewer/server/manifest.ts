...
// A pillar entry as emitted by summarizePermissions. The `describe` callback
// is stripped from the final output (it's an implementation detail used only
// to fill tone+summary just below the array literal).
interface Pillar {
  id: string;
  label: string;
  manifestKey: string;
  ...
  forbidden?: string[];
  tlsOnly?: boolean;
  describe?: (allowed: string[], forbidden: string[] | undefined, ctx: Pillar) => { tone: string; text: string };
  tone?: string;
  summary?: string;
}

function summarizePermissions(manifest: Record<string, unknown> | null | undefined) {
  const m = (manifest || {}) as Record<string, unknown>;
  const mode = ((m.mode as string) || 'enforce').toString();
  const pillars: Pillar[] = [
    {
      ...
      describe(allowed: string[], _forbidden: string[] | undefined, ctx: Pillar) {
    ...
    },
    {
      ...
      describe(_allowed: string[], forbidden: string[] | undefined) {
...
async function loadPermissions(scenarioName: string, manifestsDir: string) {
  const candidates = [
